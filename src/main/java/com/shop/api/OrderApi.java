...
public class OrderApi {
	...
	public ResponseEntity<Order> checkout(@PathVariable("email") String email, @RequestBody Cart cart) {
		...
		for(CartDetail i : items) {
			amount += i.getPrice();
			shippingFee = i.getCart().getShippingFee();
		}
		Order order = orderRepository.save(new Order(0L, new Date(),  amount + shippingFee, cart.getAddress(), cart.getPhone(), 0, "" , userRepository.findByEmail(email).get()));
		for(CartDetail i : items) {
...
//		cartDetailRepository.deleteByCart(cart);
	...
	@PostMapping("cancelReason/{orderId}")
	public ResponseEntity<Void> cancelWithReason(@PathVariable("orderId") Long id, @RequestBody String reason) {
		if(!orderRepository.existsById(id)) {
			return ResponseEntity.notFound().build();
		}
		Order order = orderRepository.findById(id).get();
